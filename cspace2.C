			  ...
			  FSMap &fsmap, SFSMap &sfmap)
{
  ...
  Faces fa = intersectionCandidates(a, octree);
  bool flag = false;
  while (!fa.empty()) {
    Face *f = *fa.rbegin();
    fa.pop_back();
...
Faces intersectionCandidates (Polyhedron *a, FOctree *octree)
{
  ...
  int n = nodes.size();
  TDataI *td = new TDataI [n];
  for (int i = 0; i < n; ++i)
    td[i].octree = nodes[i];
  pthread_t *threads = new pthread_t [n];
  Parameter::handleSignException = false;
  Parameter::increaseHighPrecision = false;
  for (int i = 0; i < n; ++i)
    pthread_create(threads + i, 0,
		   (void* (*)(void*)) intersectionCandidatesT, (void *) (td + i));
  for (int i = 0; i < n; ++i)
    pthread_join(threads[i], 0);
  Parameter::handleSignException = true;
  Parameter::increaseHighPrecision = true;
  Faces fa;
  FaceSet fs;
  for (int i = 0; i < n; ++i)
    for (Faces::iterator f = td[i].fa.begin(); f != td[i].fa.end(); ++f)
...
void intersectionCandidatesT (void *ptr)
{
  TDataI *td = (TDataI *) ptr;
  Faces fa, fb;
  td->octree->pairs(fa, fb);
  for (int i = 0; i < fa.size(); ++i)
    if (intersectsT(fa[i], fb[i])) {
      td->fa.push_back(fa[i]);
      td->fa.push_back(fb[i]);
    }
}

bool intersectsT (Face *f, Face *g)
{
  try {
...
void splitSlabs (Face *f, Face *g, CPVMap &pvmap, EPMap &epmap, Polyhedron *a,
		 FSMap &fsmap, SFSMap &sfmap, FOctree *octree, Faces &fa)
{
