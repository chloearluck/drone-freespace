...
IloBool IlosNoDuplicates(IloEnv env, IloIntervalVarArray intervals);
...
#endif
    IloAssert(IlosNoDuplicates(env, intervals), "IloIntervalVarArray: forbidden duplicate element."); 
    IloAssert(IlosNoDuplicates(env, domain), "IloIntervalVarArray: forbidden duplicate element."); 
...
public:
  ...
  IloMultipleSpan(const IloEnv env, 
    ...
    IloAssert(intervals.getImpl(), "IloIntervalVarArray: empty handle.");
    IloAssert(map.getImpl(), "IloIntExprArray: empty handle.");
    IloAssert(intervals.getSize() == map.getSize(), "IloIntExprArray: dad mapping size.");
#ifndef NDEBUG
    for (IloInt i = 0; i < domain.getSize(); i++) {
