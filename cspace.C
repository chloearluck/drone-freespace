...
{
  PV3 p = v->getP()->getApprox(1e-6), t = e->getT()->getP()->getApprox(1e-6),
    h = e->getH()->getP()->getApprox(1e-6), u = h - t, w = t.cross(u);
  if (aflag) {
    a.x = - p.x;
    a.y = - p.y;
    b.x = (p.z*u.x - w.y)/u.z;
...
void Spiral::bboxTP (Angle *as, Angle *ae, double *bbox)
{
  ...
  for (int i = 0; i < 4; ++i) {
    PTR<Angle> ti = new InputAngle(t, false);
    if (inInterval(ti, as, ae)) {
      PV2 p = SpiralPoint(ti, this).getApprox(1e-6);
